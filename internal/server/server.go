...
// ComputeFunc computes a dignity convergence report for the given birth data
...
func Run(port int, staticFS fs.FS, compute ComputeFunc, aspects AspectFunc) error {
	mux := http.NewServeMux()

	// Static files
	mux.Handle("/static/", http.StripPrefix("/static/", http.FileServer(http.FS(staticFS))))

	// API: dignity convergence
